...
import Productlists from "./Usercards";
import Navbar from "./navbar";
import {getAllUsers} from "../features/AllUsers";

export function UsersHome() {
    const dispatch = useDispatch();
    const { allusers, status, error, currentPage, totalPages } = useSelector(
    ...
    const handlePreviousPage = () => {
        if (currentPage > 1) {
            ...
            dispatch(getAllUsers(previousPage))
                .then((response) => {
                    console.log("products current page:", response.payload);
                })
                .catch((error) => console.log("ViewProduct error:", error));
    ...
    return (<div>
            ...
            <div className="mt-1 decoration-0 p-0">
                ...
                <div className="grid gridproduct-cols-1 sm:grid-cols-5 gap-1 w-screen">
                    {allusers ? (
                        allusers.data.map((allusers) => <Productlists allusers={allusers} key={allusers.id} />)
                    ) : (
